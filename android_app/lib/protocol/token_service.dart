...
class TokenService {
  ...
  final TokenRepository _repo = TokenRepository.instance;
  final RootShell _root = RootShell.instance;

  /// 从 miio2.db 扫描设备列表
  Future<MiioDbResult> scanLocalDevices() async {
    final hasRoot = await _root.checkRoot();
    if (!hasRoot) {
      AppLogger.instance.w('TokenService', 'No Root available');
